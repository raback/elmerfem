...
100 IF( nsize == 0 ) THEN
  ...
  SUBROUTINE BlockInitMatrix( Solver, BlockMatrix, BlockDofs, FieldDofs, SkipVar )
    ...
    IF( PRESENT( SkipVar ) ) THEN
      IF( SkipVar ) THEN
        CALL Info(Caller,'Skipping creation of block variables',Level=10)
    ...
    IF (isParallel) THEN
      ...
      DO RowVar=1,NoVar
        DO ColVar=1,NoVar
          ...
          ELSE
            CALL Info('BlockSolverInt',&
                'Block matrix is not square matrix - cannot initialize parallel structures!',Level=20)
          END IF
        END DO
      END DO
