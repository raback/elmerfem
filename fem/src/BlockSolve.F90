...
!        CALL CRS_SortMatrix( Amat, .TRUE. )        
  ...
  SUBROUTINE BlockPickMatrixPerm( Solver, BlockIndex, NoVar )
    ...
    CALL Info('BlockPickMatrixPerm','Picking block matrices using permutation info',Level=10)

    IF(NoVar==0) CALL Fatal('BlockPickMatrixPerm','NoVar is zero?')
    ...
    DO i = 1, NoVar
      ...
      IF(.NOT. ASSOCIATED(B)) THEN
        ...
        CYCLE
      END IF
      IF( B % NumberOfRows ==  0 ) OldMatrix = .FALSE. 
    END DO
    
    IF( OldMatrix ) THEN
      CALL Info('BlockPickMatrixPerm','Using old matrix structures!',Level=6)
    ELSE
      CALL Info('BlockPickMatrixPerm','Creating new matrix structures!',Level=12)
    END IF
    ...
    DO i = 1, NoVar
      ...
      DO j = 1, NoVar
        ...
        CALL Info('BlockPickMatrixPerm','Doing matrix '//I2S(10*i+j)//&
            ' of size '//I2S(n)//' x '//I2S(rowcount(j)))

        IF(OldMatrix) THEN
          B % Values = 0.0_dp
        ELSE
          ! Add the (n,n) entry since this helps to create most efficiently the full ListMatrix
          ! All PDE-based equations should have the extreme entry.
          CALL AddToMatrixElement(B,n,rowcount(j),0.0_dp)      
        END IF
